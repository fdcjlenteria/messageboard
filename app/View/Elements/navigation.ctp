...
<?php
	// to get the updated profile pic of the auth user
	$userPhotoUrl = $this->requestAction('users/getAuthPhotoUrl/' . AuthComponent::user('id'));
	if (empty($userPhotoUrl)) {
		$photoUrl = 'https://cdn-icons-png.flaticon.com/512/147/147142.png';
	} else {
		$photoUrl = $this->Html->webroot($userPhotoUrl);
	}
?>
